			...
			@if ($projectsActive->isNotEmpty())
			  ...
			  <div class="swiper-container px-4">
			    <div class="swiper-wrapper">
					...
					@foreach ($projectsActive as $index => $projectsActiveSignleRow)
							...
							<div class="swiper-slide">
							  ...
							  <div class="card border-0 text-center">
								  <img src="{{ $projectsActiveSignleRow->header_image_url }}" alt="{{ $projectsActiveSignleRow->cover_image_description }}" class="rounded-15">
                    ...
                    <div class="border project-card-box p-2 mt-2 rounded-15">
                      ...
                      <span class="d-flex flex-column px-2 mt-2">
                        <p class="font-weight-bold text-left mt-2">Cilj</p>
                          <div class="progress mx-4 mt-n2 rounded-15">
                            <div class="progress-bar progress-bar-striped progress-bar-animated rounded-15" role="progressbar" style="width: {{  $projectsActiveSignleRow->percentage . '%' }}" aria-valuenow="300" aria-valuemin="0" aria-valuemax="10100"></div>
                          </div>	
                          <div class="mt-4 mx-4 d-flex justify-content-between align-items-start">
                            <span class="text-left">
                              <span class="py-2 px-3 rounded-15 money-amount">{{ $projectsActiveSignleRow->money_collected }}</span>
                            </span>
                            <span>&mdash;</span>
                            <span>
                              <span class="py-2 px-3 rounded-15 money-amount">{{ $projectsActiveSignleRow->money_goal }}</span>
                            </span>
                          </div>	 	    		
                      </span>
                  ...
                  @endif
								  ...
								  <a href="{{ Route('specificProject', ['project' => $projectsActiveSignleRow->title_slug]) }}" class="btn rounded-15 px-4 py-2 mt-3 text-white">Saznaj više</a>
								  </div>
							  </div>
							  <!-- /.Card -->
			    ...
			    <div class="swiper-wrapper">
					...
					@foreach ($projectsPassed as $index => $projectsPassedSingleRow)
							...
							<div class="swiper-slide">
								...
								<!-- Card -->
								<div class="card border-0 text-center">
									<img src="{{ $projectsPassedSingleRow->header_image_url }}" alt="{{ $projectsPassedSingleRow->cover_image_description }}" class="rounded-15">
									<div class="card-body px-0">
										...
										<a href="{{ Route('specificProject', ['project' => $projectsPassedSingleRow->title_slug]) }}" class="btn rounded-15 px-4 py-2 mt-3 text-white">Saznaj više</a>
									</div>
									</div>
									<!-- /.Card -->
